SELECT
DateName(month,Entry_Date) + ' - ' +

SELECT  
    CONVERT(DATE, Sales_Invoice_Master_New.SaleDate) AS Sale_Date,
    Sales_Invoice_Master_New.InvoiceNo,
    '(' + M_Customer.Cust_Code + ') - ' + M_Customer.Cust_Name AS SalesCust_Name,

    STUFF((
        SELECT ', ' + CAST(Employee_Master.Full_Name AS VARCHAR(MAX))
        FROM dbo.Employee_Master EM
        WHERE Employee_Master.Emp_ID IN (
            SELECT SalesMan_ID
            FROM dbo.SalesMan_Master
            WHERE Is_Status = 1
              AND Sales_Invoice_Master_New.Cust_ID IN (SELECT * FROM dbo.HDSplit(CustomerIDs))
        )
